...
lines = [
    record.description.strip().split("|")
    for record in SeqIO.parse(open(sys.argv[2], "r"), "fasta")
]
...
for line in lines:
    if line[1] == "SLE":
        ...
        if m:
            ...
            line.append(line[2])
        newlist.append(
            ["EBOV", line[0], "Goodfellow", "SLE", line[2] + "--", line[3], line[4]]
        )
    else:
...
for line in newlist:
    ...
    string = ",".join(
        (
            line[1],
            line[1],
            "%.7f" % (locations[prefec][0]),
            "%.7f" % (locations[prefec][1]),
            prefec,
            shape,
            colourDict[prefec],
            line[5],
