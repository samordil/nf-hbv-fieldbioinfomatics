...
def go(args):
    ...
    for depthFile in args.depthFiles:
        ...
        df = pd.read_csv(
            ...
            sep="\t",
            header=None,
            names=["refName", "readGroup", "position", "depth"],
            dtype={"refName": str, "readGroup": str, "position": int, "depth": int},
            usecols=(0, 1, 2, 3),
        )
        ...
        assert len(df.index) == (
            (endPos - startPos) + 1
        ), "error: depth needs to be reported at all positions"
        ...
        rg = rgList[0]
        assert rg in rgAmplicons, (
            "error: readgroup not found in provided primer scheme (%s)" % (rg)
        )
        ...
        df["amplicon"] = pd.cut(x=df["position"], bins=starts, labels=amplicons)

        # store the mean of each bin
        bins = (df.groupby(["amplicon"])["depth"].mean()).rename(depthFile.name)

        # add to the pile
        assert rg not in dfs, "error: readgroup present in multiple files (%s)" % (rg)
        dfs[rg] = bins
    ...
    newDF = newDF.melt(
        "amplicon", var_name="read group", value_name="mean amplicon read depth"
    )
    newDF = newDF.dropna()
    ...
    # plot the box
    g = sns.catplot(
        data=newDF, x="read group", y="mean amplicon read depth", kind="box"
    )
    g.fig.suptitle(args.sampleID)
...
def main():
    ...
    parser.add_argument("--primerScheme", required=True, help="the ARTIC primer scheme")
    parser.add_argument(
        "--sampleID", required=True, help="the sample ID for the provided depth files"
    )
    parser.add_argument(
        "--outFilePrefix",
        default="./amplicon-depth",
    ...
    parser.add_argument(
        ...
        nargs="+",
        help="the depth files produced by make_depth_mask.py",
    )
