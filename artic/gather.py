...
def run(parser, args):
    if not args.directory:
        if not os.path.exists(args.prompt_directory):
            print(
                "Please specify a directory with --directory, artic gather --help for details."
            )
            raise SystemExit
        directories = os.listdir(args.prompt_directory)
        directories = [
            args.prompt_directory + "/" + d
            for d in directories
            if os.path.isdir(args.prompt_directory + "/" + d)
        ]
    ...
    if isinstance(args.directory, list) and len(args.directory) > 1 and not args.prefix:
        print(
            "Must supply a prefix if gathering multiple directories!", file=sys.stderr
        )
        raise SystemExit(1)
    ...
    for directory in args.directory:
        ...
        for root, dirs, files in os.walk(d):
            ...
            fastq[barcode_directory].extend(
                [root + "/" + f for f in files if f.endswith(".fastq")]
            )
            summary_files.extend(
                [
                    root + "/" + f
                    for f in files
                    if fnmatch.fnmatch(f, "*cing_summary*txt")
                ]
            )
    ...
    for barcode_directory, fastq in list(fastq.items()):
        if len(fastq):
            ...
            print(
                "Processing %s files in %s" % (len(fastq), barcode_directory),
                file=sys.stderr,
            )
    ...
    if dfs:
        ...
        print(
            "No sequencing summary files found. This may be because the run is ongoing. You can proceed but nanopolish index will be slow and may not be able to use all of your data."
        )
