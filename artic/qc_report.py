...
def create_qc(mappingstats: str, consensus: str, output: str):
    ...
    coverage_percent = round(
        (
            (len(consensus_genome) - consensus_genome.seq.count("N"))
            / len(consensus_genome)
        )
        * 100,
        2,
    )
...
def main():
    ...
    parser.add_argument(
        "--mappingstats", type=str, required=True, help="Mapping stats file"
    )
    parser.add_argument(
        "--consensus", type=str, required=True, help="Mapping stats file"
    )
    parser.add_argument(
